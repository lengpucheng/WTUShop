...
public class UserInfoController {
    ...
    public String doRegister(UserInfo userInfo, HttpServletResponse response) {
        ...
        if (register==0) {
            ...
            try {
                response.getWriter().write(JSTools.alterUrl("注册成功！","/login"));
            } catch (IOException e) {
                e.printStackTrace();
            }        }
        return null;
...
//    /user/space/
    ...
    public String doEdit(UserInfo userInfo,HttpServletRequest request,HttpServletResponse response){
        ...
        if(edit==-1){
            ...
            try {
                ...
                response.getWriter().write(JSTools.alterUrl("修改成功！请重新登陆！","/login"));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public String signOut(HttpServletRequest request,HttpServletResponse response){
        ...
        response.setContentType("text/html; charset=utf-8");
        try {
            response.getWriter().write(JSTools.alterReplace("已安全退出","/"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
